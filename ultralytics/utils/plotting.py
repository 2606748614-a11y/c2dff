...
def calculate_mean_correlation(correlation_matrix):
    """计算相关系数矩阵的平均相关性（不包括对角线）"""
    mask = np.ones(correlation_matrix.shape) - np.eye(correlation_matrix.shape[0])
    return np.sum(correlation_matrix * mask) / np.sum(mask)
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    if isinstance(x, torch.Tensor):
        ...
        if height > 1 and width > 1:
            # Prepare the save path for the feature visualization image
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features.png"

            # Create blocks of features (one for each channel)
            ...
            # Visualize feature maps for each channel
            for i in range(n):
                ax[i].imshow(blocks[i].squeeze(), cmap='gray')  # Show each feature map in grayscale
                ax[i].axis("off")
            ...
            # Save the feature map as a .npy file for further analysis
            np.save(str(f.with_suffix(".npy")), x[0].cpu().numpy())  # npy save

            # Calculate the Pearson correlation matrix
            correlation_matrix = np.zeros((channels, channels))

            # Compute Pearson correlation for each pair of channels
            for i in range(channels):
                for j in range(i, channels):
                    corr = pearson_corr(blocks[i].squeeze(), blocks[j].squeeze())
                    correlation_matrix[i, j] = corr
                    correlation_matrix[j, i] = corr  # Symmetric matrix

            # Calculate mean correlation
            mean_correlation = calculate_mean_correlation(correlation_matrix)

            # Set color mapping range to [-1, 1]
            vmin = -1
            vmax = 1

            # Visualize the Pearson correlation matrix with unified color mapping
            corr_f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_pearson_corr_{mean_correlation:.4f}.png"
            fig, ax = plt.subplots(figsize=(8, 8))
            cax = ax.imshow(correlation_matrix, cmap='coolwarm', interpolation='nearest', vmin=vmin, vmax=vmax)
            fig.colorbar(cax)
            ax.set_title(f"Pearson Correlation Matrix - Stage {stage}")
            plt.savefig(corr_f, dpi=300, bbox_inches="tight")
            plt.close()

            LOGGER.info(f"Pearson Correlation Matrix saved to: {corr_f}")
