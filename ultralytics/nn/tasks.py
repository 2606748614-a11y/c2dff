...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        ...
        elif isinstance(m, nn.GELU) and not hasattr(m, "approximate"):
            m.approximate = "none"  # compatibility with GELU modules saved by older torch builds
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c1, c2 = ch[f], args[0] #c1=ch[f]代表将上一个模块的输出通道作为此模块的输入通道 这里的args是yaml文件里的
            ...
            if m is C2fAttn:
                ...
                args[2] = int(
                  ...
                  # num heads
            args = [c1, c2, *args[1:]] #重新构造了args列表，打印出来
        ...
        #####attention  ####
            ...
            c1 = ch[f[0]] #输入的是两个通道（R+T），之所以C1只等于一个通道的，是因为foword函数里设置的是通道add # x = torch.add(x[0], x[1])
            #print(c11)
            c2 = args[0]
            if c2 != nc:
                #c2 = make_divisible(min(c2, max_channels) * width, 8) #C2不为指定通道数，取决与scale
                c2 = make_divisible(min(c2, max_channels)* width, 8)  # C2不为指定通道数，取决与scale
            #print(c2)
            #args = [c1, c2]
            args = [c1, c2, *args[1:]]#指控制台输出网络结构的那个args
        elif m is PolarizedSelfAttention:
            c1 = ch[f[0]] #输入的是两个通道（R+T），之所以C1只等于一个通道的，是因为foword函数里设置的是通道add # x = torch.add(x[0], x[1])
            #print(c11)
            c2 = args[0]
            if c2 != nc:
        ...
        #####attention  ####
            ...
            c1 = ch[f[0]] + ch[f[1]]  # input channels f=[X,X]  #inputs = torch.cat((x[0], x[1]), dim=1) #把输入的可见光、红外图像通道级联
            c2 = c1 // 2
            args = [c1, c2]
        # elif m is CPCA:#若是做差分
        #     c1 = ch[f[0]] # input channels f=[X,X]  #inputs = torch.cat((x[0], x[1]), dim=1) #把输入的可见光、红外图像通道级联
        #     c2 = c1
        #     args = [c1, c2]
        elif m is FeatureAdd:
            c2 = ch[f[0]]#输出通道=输入通道
        elif m in {CPCA_CF, CDFIM}:
            c1 = ch[f[0]]  # 输入的是两个通道（R-T），之所以C1只等于一个通道的
            c2 = ch[f[0]]#输出通道=输入通道
            args = [c1, c2]
